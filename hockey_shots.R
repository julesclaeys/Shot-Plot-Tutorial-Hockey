...
# Add off target shot points
    geom_point(data = subset(sub_data, event == "SHOT"), aes(x = arenaAdjustedXCord, y = arenaAdjustedYCord), size = 0.5, color = "red") +
...
# Add the Goal points
    geom_point(data = subset(sub_data, event == "GOAL"), aes(x = arenaAdjustedXCord, y = arenaAdjustedYCord), size = 1, color = "blue") +
    labs(title = unique(sub_data$Opponent))
})

# Plot the charts together in 4 columns
